...
def same_business_day_last_year(latest_date: pd.Timestamp) -> pd.Timestamp:
    ...
    - ambil tanggal kalender yang sama satu tahun sebelumnya;
    - bila jatuh pada akhir pekan, mundurkan ke business day sebelumnya;
    - baru kemudian dicari exact match pada data.

    Jika exact match tidak ada (mis. hari libur pasar), hasil YoY ditampilkan '-'.
    """
    target = latest_date - pd.DateOffset(years=1)
    # geser ke hari kerja sebelumnya bila weekend
...
def base_layout(fig: go.Figure, title: str, subtitle: str | None = None, height: int = 320):
    fig.update_layout(
        title={
...
def sbn_ust_chart(df: pd.DataFrame):
    ...
    fig = base_layout(fig, "Yield SBN 10Y dan UST 10Y")
...
subtitle = f"Closing per {latest_date.strftime('%d %b %Y')}"
...
st.plotly_chart(sbn_ust_chart(plot_df), use_container_width=True)
render_table(summary_table(df, [
